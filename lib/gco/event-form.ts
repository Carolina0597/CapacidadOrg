...
export interface EventModel {
  ...
  // Paso 2
  tipo: ActivityType | ""
  /** Texto obligatorio cuando el tipo seleccionado es "Otras". */
  tipoOtro: string
  subtipo: string
  prioridad: string
  peso: string
  ...
  requiereAprobacionInvitados: boolean
  correoAprobadorInvitados: string
  instruccionesAprobacionInvitados: string
  personasAprobacionCorreos: string[]
...
export function emptyEventModel(id: string, periodo: Period, user: User): EventModel {
  return {
    ...
    requiereAprobacionInvitados: false,
    correoAprobadorInvitados: "",
    instruccionesAprobacionInvitados: "Validar disponibilidad y autorizar asistencia.",
    personasAprobacionCorreos: [],
...
export function detectConflicts(m: EventModel): Conflict[] {
  const out: Conflict[] = []
  const overlap = (aF:string,aI:string,aE:string,bF:string,bI:string,bE:string) => !!aF && aF===bF && aI<bE && bI<aE
  const invited = new Set((m.personas || []).filter(p=>p.valido).map(p=>p.correo.toLowerCase()))
  m.sesiones.forEach((s) => {
    if (!s.fecha) return
    EXISTING_CROSS.forEach((x) => { if (overlap(s.fecha,s.ini,s.fin,x.fecha,x.ini,x.fin)) out.push({ sesion:s.nombre, tipo:"externo", contra:x.name, detalle:`${x.fecha} · ${x.ini}–${x.fin}` }) })
    m.sesiones.forEach((o) => { if (o.id>s.id && overlap(s.fecha,s.ini,s.fin,o.fecha,o.ini,o.fin)) out.push({ sesion:s.nombre,tipo:"interno",contra:o.nombre,detalle:`${o.fecha} · ${o.ini}–${o.fin}` }) })
    const individual=[
      {correo:"laura.roldan@sistecredito.com",fecha:"2026-08-20",ini:"08:00",fin:"10:00",evento:"Comité de Riesgos"},
      {correo:"daniel.marin@sistecredito.com",fecha:"2026-08-20",ini:"10:00",fin:"12:00",evento:"Entrenamiento operativo"},
      {correo:"sandra.roldan@sistecredito.com",fecha:"2026-08-21",ini:"14:00",fin:"16:00",evento:"Mesa de operación"},
    ]
    individual.forEach(x=>{if(invited.has(x.correo)&&overlap(s.fecha,s.ini,s.fin,x.fecha,x.ini,x.fin)) out.push({sesion:s.nombre,tipo:"persona",contra:x.correo,detalle:`${x.correo} ya tiene “${x.evento}” · ${x.ini}–${x.fin}`})})
  })
  return out
}
...
export function validateStep(step: number, m: EventModel): Errors {
  ...
  if (step === 1) {
    ...
    if (m.tipo === "otras" && !(m.tipoOtro || "").trim()) e.tipoOtro = "Especifica el tipo de actividad"
  ...
  if (step === 4) {
    ...
    if (!m.requiereInscripcion && m.requiereAprobacionInvitados && !(m.correoAprobadorInvitados || "").includes("@")) e.correoAprobadorInvitados = "Ingresa un correo válido"
    if (!m.requiereInscripcion && m.requiereAprobacionInvitados && !(m.personasAprobacionCorreos || []).length) e.personasAprobacionCorreos = "Selecciona al menos una persona"
  }
  return e
}
