...
function notif(
  ...
  return [{ id: uid("N"), fecha: nowISO(), leida: false, fallida: false, ...n }, ...s.notifications]
}
...
export const A = {
  ...
  markAllRead(role: Role) {
    commit({
      ...state,
      notifications: state.notifications.map((n) => (n.destinatarioRol === role ? { ...n, leida: true } : n)),
    })
  },
  /**
  ...
  saveEvent(model: Record<string, any>, actor: string, isDraft: boolean): GcoEvent {
    const estado = isDraft ? "borrador" : "enviado"
  ...
  resubmitEvent(id: string, actor: string) {
    const ev = state.events.find((e) => e.id === id)
    if (!ev) return
    const tz = [...ev.traza, { actor: USERS[actor]?.nombre || actor, ts: nowISO(), from: ev.estado, to: "enviado", comentario: "Reenvío tras ajuste" }]
    commit({
      ...state,
      events: state.events.map((e) => (e.id === id ? { ...e, estado: "enviado", updatedAt: nowISO(), traza: tz } : e)),
      audit: audit(state, { actor, action: "event.resubmit", entity: "event", entityId: id, after: "enviado" }),
      notifications: notif(state, {
        kind: "evento_recibido",
        destinatarioRol: Role.ForPlus,
  ...
  requestApproval(eventId: string, enrollmentIds: string[], actor: string, opts: Partial<ApprovalRequest>) {
    ...
    commit({ ...state, approvals: [req, ...state.approvals], enrollments: state.enrollments.map((x) => enrollmentIds.includes(x.id) ? { ...x, approvalState: "pendiente" } : x), events: state.events.map((e) => e.id === eventId ? { ...e, estado: "en_aprobacion" } : e), audit: audit(state, { actor, action: "approval.request", entity: "approval", entityId: req.id, after: "pendiente" }), notifications: notif(state, { kind: "aprobacion", destinatarioRol: Role.Lider, titulo: "Aprobación formal requerida", descripcion: `Tienes ${enrollmentIds.length} persona(s) por aprobar.`, requiereAccion: true, rel: req.id }) })
    return req
  },
  respondApproval(id: string, approver: string, decision: "aprobada" | "rechazada", observacion?: string) {
    const req = state.approvals.find((x) => x.id === id); if (!req) return
    const respuestas = [...req.respuestas.filter((r) => r.approver !== approver), { approver, decision, ts: nowISO(), observacion }]
    ...
    const estado = rejected ? "rechazada" : approved ? "aprobada" : "pendiente"
...
export const sel = {
  ...
  eventsToReview: (s: GcoState) =>
    s.events.filter((e) => ["enviado", "en_revision"].includes(e.estado)),
  kpis: (s: GcoState) => ({
    registrados: s.events.length,
    porRevisar: s.events.filter((e) => ["enviado", "en_revision"].includes(e.estado)).length,
    priorizacion: s.events.filter((e) => e.estado === "pendiente_priorizacion").length,
  ...
  governance: (s: GcoState) => {
    const evs = s.events
    const active = (e: GcoEvent) => ["enviado", "en_revision", "pendiente_priorizacion"].includes(e.estado)
    const towardPublish = (e: GcoEvent) => ["pendiente_priorizacion", "aprobado", "publicado"].includes(e.estado)
    const periodo = s.session.periodo

    const indicadores = {
      registrados: evs.length,
      porRevisar: evs.filter((e) => ["enviado", "en_revision"].includes(e.estado)).length,
      priorizacion: evs.filter((e) => e.estado === "pendiente_priorizacion").length,
    ...
    const vencidosANS = evs.filter(
      (e) => ["enviado", "en_revision"].includes(e.estado) && daysBetween(TODAY, e.createdAt) > ANS_DIAS_REVISION,
    ).length
  ...
  team: (s: GcoState) => {
    ...
    const own = TEAM.filter((m) => m.lider === nombre)
    const members = own.length ? own : TEAM
  ...
  notifsFor: (role: Role) => (s: GcoState) => s.notifications.filter((n) => n.destinatarioRol === role),
  unread: (role: Role) => (s: GcoState) =>
    s.notifications.filter((n) => n.destinatarioRol === role && !n.leida).length,
}
