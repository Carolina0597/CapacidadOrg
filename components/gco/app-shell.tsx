...
import { PERIODOS, Role, ROLE_LABEL, USERS, type Role as RoleType } from "@/lib/gco/domain"
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const unread = useGco(sel.unreadFor(role, user?.correo || ""))
  ...
  const activeKey = routeByHref(pathname)?.key ?? (role === Role.Colaborador || role === Role.Lider ? "miCapacidad" : role === Role.AreaPromotora ? "calendario" : "gestion")

  const changeRole = (r: RoleType) => {
    const demoByRole: Record<RoleType, string> = {
      [Role.Colaborador]: "daniel",
      [Role.Lider]: "alejandro",
      [Role.AreaPromotora]: "carolina",
      [Role.ForPlus]: "valeria",
    }
    if (!user.roles.includes(r)) A.login(demoByRole[r])
    A.setRole(r)
    setRoleOpen(false)
    const target = r === Role.Colaborador || r === Role.Lider ? "/capacidad" : r === Role.AreaPromotora ? "/calendario" : "/gestion-de-eventos"
    router.push(target)
  }
  const switchUser = (id: string) => {
    A.login(id)
    setLoginOpen(false)
    const preferred: Record<string, RoleType> = { daniel: Role.Colaborador, alejandro: Role.Lider, carolina: Role.AreaPromotora, valeria: Role.ForPlus }
    const rr = preferred[id] ?? USERS[id]?.roles[0] ?? Role.Colaborador
    A.setRole(rr)
    router.push(rr === Role.Colaborador || rr === Role.Lider ? "/capacidad" : rr === Role.AreaPromotora ? "/calendario" : "/gestion-de-eventos")
  }
  ...
  return (
    <div className="gco min-h-screen flex">
          ...
          <div className="relative" ref={perRef}>
            ...
            {perOpen && (
              <div className="absolute right-0 mt-1.5 w-44 py-1.5 pop" style={{ ...cardWhite, zIndex: 40 }}>
                ...
                {!user.roles.includes(Role.Lider) && <button type="button" onClick={()=>{A.login("alejandro");A.setRole(Role.Lider);setRoleOpen(false);router.push("/capacidad")}} className="fx w-full px-3 py-2 text-left text-sm border-t hover:bg-[color:var(--gco-surface-2)]"><span className="block font-semibold">Ver demostración del perfil Líder</span><span className="block text-[10px]" style={{color:"var(--gco-text-3)"}}>Cambia a Alejandro Gómez para validar Mi equipo y aprobaciones.</span></button>}
              </div>
            )}
          </div>
          ...
          <div className="relative" ref={roleRef}>
            ...
            {roleOpen && (
              <div className="absolute right-0 mt-1.5 w-56 py-1.5 pop" style={{ ...cardWhite, zIndex: 40 }}>
                ...
                <div className="px-3 pb-1 text-[10px]" style={{color:"var(--gco-text-3)"}}>Tu vista personal de colaborador y los roles adicionales asignados.</div>
                {user.roles.map((r) => (
                  <button
                    key={r}
                    type="button"
                  ...
                  >
                    <span>
                      ...
                      <span className="block text-[10px]" style={{ color: "var(--gco-text-3)" }}>{r===Role.Colaborador?"Mi experiencia personal":`Rol adicional · ${user.area}`}</span>
                    </span>
      ...
      <Modal open={loginOpen} onClose={() => setLoginOpen(false)} title="Cambiar usuario de demostración" w={420}>
        ...
        {["daniel", "alejandro", "carolina", "valeria"].map((id) => USERS[id]).filter(Boolean).map((x) => (
          <button
            key={x.id}
            type="button"
